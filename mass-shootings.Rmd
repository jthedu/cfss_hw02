...
#ask why don't need to do NA above - think it's cuz there are no NA values for months. can still have drop_na,
#but it doesn't make a difference in end result
#######also, drop_na doesn't change original dataframe, correct?????
# DELETE BELOW!!!!!
...
mass_shootings %>%
  ...
  labs(title = "Mass shootings in the U.S. (1982-2019)", x = "Number of fatalities per incident", y = "Number of incidents") +
  facet_grid(race ~ ., scales = "free") 
### kinda unsure why 2nd chuck works (& 1st doesn't). also can i pipe 2 ggplots in a row? ASK ABOUT THIS

#attempt to pipe both graphs????? or create a new pipe for 2nd graph?
###also need to answer this q for final q - cuz otherwise have to keep rewriting pipe (to get all the diff graphs), i think....
...
ggplot(data = na.omit(noLV[, c("race", "total_victims")]), mapping = aes(x = race, y = total_victims)) + 
  geom_boxplot() +
  labs(title = "Mass shootings in the U.S. (1982-2019)", x = "Race of perpetrator", y = "Total number of victims")
###could i use outlier.case as alternative????
